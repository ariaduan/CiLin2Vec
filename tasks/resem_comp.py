...
fileout = open('../results/resem_comp_cb_cilin_def_palin_3','w',encoding = 'utf-8')#replace this file in correspondence with the name of file1
...
for line2 in file2:
    ...
    for i in range(len(sem)-1,-1,-1):
        ix = bank.index(sem[i])
        tmp = multi_vec(vec[ix],1/(2**(len(sem)-i))) if (i != 0) else multi_vec(vec[ix],1/(2**(len(sem)-i-1)))
