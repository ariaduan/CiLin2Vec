...
for model in models:
    ...
    for sem1 in MC_sem1:
        ...
        for i in range(len(sem1)):
            tmp = multi_vec(embeddings[sem1[i]],1/(2**(len(sem1)-i))) if (i != (len(sem1)-1)) else multi_vec(embeddings[sem1[i]],1/(2**(len(sem1)-i-1)))
            semvec1 = semvec1 + np.mat(tmp)
        for i in range(len(sem2)):
            tmp = multi_vec(embeddings[sem2[i]],1/(2**(len(sem2)-i))) if (i != (len(sem2)-1)) else multi_vec(embeddings[sem2[i]],1/(2**(len(sem2)-i-1)))
